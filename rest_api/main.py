...
def start_rest_api(host, port, messenger, database):
    ...
    LOGGER.info('Validator successfully connected')

    handler = RouteHandler(loop, messenger, database)

    app.router.add_post('/getpk',handler.get_public_key)
    ...
    app.router.add_post('/records/{record_id}/finalize', handler.finalize_record)

    app.router.add_get('/fields/{role}', handler.fetch_fields)
